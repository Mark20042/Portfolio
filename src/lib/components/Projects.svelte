<script lang="ts">
	...
	// Grab all items explicitly marked as featured, or fallback to the first 6
	const explicitlyFeatured = projectsData.filter(p => p.featured);
	const featuredProjects = explicitlyFeatured.length > 0 ? explicitlyFeatured : projectsData.slice(0, 6);
...
<div class="mb-24 pt-12 sm:pt-16" id="projects">
	<div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-12 sm:mb-16">
		...
		<a href="/projects" class="inline-flex items-center gap-2 text-sky-500 hover:text-sky-600 dark:hover:text-sky-400 font-semibold transition-colors group whitespace-nowrap pb-1 sm:pb-2">
			View All Projects
	...
	<div class="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 mb-12">
		{#each featuredProjects as project}
			<button 
				...
				class="relative w-full aspect-[4/3] rounded-3xl overflow-hidden group shadow-lg hover:shadow-2xl hover:-translate-y-1 transition-all duration-500 bg-slate-100 dark:bg-[#111] border border-slate-200 dark:border-[#222] text-left cursor-pointer"
			>
				<!-- Image Section -->
